...
import io.polyaxis.api.utils.exceptions.ApplicationException;
...
public final class ClassUtils {
    ...
    public static Class<?> findClassByName(String className) {
        try {
            ...
            throw new ApplicationException(500, "Class name \"" + className + "\" not found!");
        }
    }
    ...
    /// Replacement for `Class.forName()` that also returns Class instances for primitives \(e.g. "int"\) and array
    ...
    /// Does <i>not</i> support the "\[\]" suffix notation for primitive arrays; this is only supported by
    /// [#forName(String, ClassLoader)].
    ///
    /// @param name the name of the potentially primitive class
    /// @return the primitive class, or `null` if the name does not denote a primitive class or primitive array
    /// class
    ...
    /// well).
    ///
    /// @return the default ClassLoader (only `null` if even the system ClassLoader isn't accessible)
    ...
    /// concatenated with a slash and the name of a resource and fed directly to `ClassLoader.getResource()`. For
    /// it to be fed to `Class.getResource` instead, a leading slash would also have to be prepended to the
    /// returned value.
    ///
    /// @param clazz the input class. A `null` value or the default \(empty\) package will result in an empty string
    /// \(""\) being returned.
